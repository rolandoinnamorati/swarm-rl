...
PLANE_SIZE = 101
MAX_STEPS = 500
EPS_RADIUS = max(1.0, 0.005 * PLANE_SIZE) # soglia "arrivo" in distanza euclidea
SEED = 0
...
class DroneEnv:
    ...
    def reset(self, episode=None) -> np.ndarray:
        if episode is None:
            max_d = self.plane
        else:
            max_d = max_spawn_manhattan_for_episode(episode, self.plane)

        tries = 0
        while True:
            d = (rng.integers(0, self.plane), rng.integers(0, self.plane))
            t = (rng.integers(0, self.plane), rng.integers(0, self.plane))
            if d != t and (abs(d[0] - t[0]) + abs(d[1] - t[1]) <= max_d):
                break
            tries += 1
            if tries > 1000:
                break  # fallback
...
def max_spawn_manhattan_for_episode(ep, plane):
    if ep < 500:    return min(20, plane)
    if ep < 2000:   return min(40, plane)
    return plane
...
def train():
    ...
    for ep in range(1, EPISODES + 1):
        obs = env.reset(episode=global_episode)
        done = False
        ep_return = 0.0
